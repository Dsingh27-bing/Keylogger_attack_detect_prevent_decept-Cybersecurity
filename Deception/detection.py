...
def terminate_process(pid):
    try:
        # Ensure that the PID is a positive integer
        pid = int(pid)
        if pid > 0:
...
# Flag to track if keyloggers are detected
                    ...
                    if len(process) > 10:
                        ...
                        for item in keylogger_name:
                            if item["name"] in process_cmd and item["name"] in info['name']:
                                ...
                                loading()
                                keyloggers_detected = True
                                #terminate_process(process[1])  #we don't want to end the process we are just fooling the attacker now
                                break
